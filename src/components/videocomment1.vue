<template>
  <div class="item">
    ...
    <div class="item-right">
      <h5>{{comment1.userVO.nickName}}</h5>
      <p>{{comment1.content}}</p>
      <div class="item-bottom">
        <span>{{timestampToTime(comment1.commentTime)}}</span>
        <em>{{comment1.favorCount}}</em>
      </div>
      <Videocomment2 v-for="(vv,i) in comment1.replyList" :key="i" :comment2="vv"/>
    </div>
  </div>
</template>

<script>
import Videocomment2 from 'com/videocomment2';
export default {
  props:["comment1"],
  components:{
    Videocomment2
